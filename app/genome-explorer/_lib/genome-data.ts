...
// Coordinates from GRCh38/hg38 RefSeq annotations.
//
// TODO [REAL GENE DB]: Replace with MANE Select annotation file for production.
// Current coords are representative — verify against Ensembl/RefSeq for exact boundaries.

export type GeneCategory = "cancer" | "cardio" | "nicu" | "rare_disease" | "pgx";
...
// Search index — built once for fast autocomplete
const GENE_INDEX = GENE_DB.map((g, idx) => ({
  idx,
  lowerSymbol: g.symbol.toLowerCase(),
  lowerName: g.name.toLowerCase(),
}));

export function searchGenes(query: string, limit = 10): Gene[] {
  const q = query.toLowerCase().trim();
  if (!q) return [];

  const scored: { gene: Gene; score: number }[] = [];
  for (const gi of GENE_INDEX) {
    let score = 0;
    if (gi.lowerSymbol === q) score = 100;
    else if (gi.lowerSymbol.startsWith(q)) score = 80;
    else if (gi.lowerSymbol.includes(q)) score = 60;
    else if (gi.lowerName.includes(q)) score = 40;
    else continue;
    scored.push({ gene: GENE_DB[gi.idx], score });
  }

  scored.sort((a, b) => b.score - a.score);
  return scored.slice(0, limit).map((s) => s.gene);
}

export function getGeneBySymbol(symbol: string): Gene | null {
  return GENE_DB.find((g) => g.symbol === symbol) ?? null;
}

export const PANEL_PRESETS: Record<string, PanelPreset> = {
  cancer: {
    name: "Cancer Predisposition",
    description: "ACMG SF v3.2 cancer genes plus commonly tested oncogenes",
    genes: GENE_DB.filter((g) => g.cat.includes("cancer")),
  },
  cardio: {
    name: "Cardiovascular",
    description: "Cardiomyopathy, arrhythmia, aortopathy, and lipid genes",
    genes: GENE_DB.filter((g) => g.cat.includes("cardio")),
  },
  neonatal: {
    name: "Neonatal / NICU",
    description: "Newborn screening and critical neonatal conditions",
    genes: GENE_DB.filter((g) => g.cat.includes("nicu")),
  },
  rare_disease: {
    name: "Rare Disease",
    description: "Mendeliome — comprehensive rare disease gene panel",
    genes: GENE_DB.filter((g) => g.cat.includes("rare_disease")),
  },
  pgx: {
    name: "Pharmacogenomics",
    description: "Drug metabolism and pharmacogenomic response genes",
    genes: GENE_DB.filter((g) => g.cat.includes("pgx")),
  },
  whole_exome: {
    name: "Whole Exome",
    description: "All catalogued genes representing an exome subset",
    genes: [...GENE_DB],
  },
};

export function genesToRegions(genes: Gene[]): BedRegion[] {
  return genes.map((g) => ({
    chr: g.chr,
    start: g.start,
    end: g.end,
...
export function parseBedFile(text: string): {
  regions: BedRegion[];
  format: string;
  warnings: string[];
} {
  ...
  for (const line of lines) {
    ...
    const name = cols.length >= 4 ? cols[3] : "";
    ...
    if (!isNaN(start) && !isNaN(end) && end > start) {
      regions.push({ chr, start, end, name });
    }
  }
  ...
  return { regions, format, warnings };
}
